...
## 2) heatmap with CTA siginificant in chondro
```{r, fig.cap="Heatmap of CTAs that impact survival (n = 53)"}
...
data <- matrix_expr_cta[rownames(matrix_expr_cta) %in% l_CTA_conv,]
...
## 3) Heatmap with CTA signif in osteo
```{r, fig.cap="Heatmap of CTAs that impact survival (n = 53)"}
# List of CTA for coxph analysis
df_CTA_osteo <- read.table("../results/results_coxph_osteo_cta_zscore_signif.tsv", header = T)
l_CTA_osteo <- df_CTA_osteo$Variable
data <- matrix_expr_cta[rownames(matrix_expr_cta) %in% l_CTA_osteo,]

# Create the heatmap
#pdf("../results/figures/heatmaps/heatmap_cta_coxph_signif_osteo.pdf", height = 5)
heatmap_cta_signif <- Heatmap(data,
    cluster_rows = TRUE,    
    cluster_columns = TRUE,
    cluster_column_slices = TRUE,
    clustering_distance_columns = "euclidean",
...
heatmap_cta_signif <- draw(heatmap_cta_signif)
#dev.off()
```
```{r}
# Take col indexes
indiv_clust <- column_order(heatmap_cta_signif)

# Create table with indiv names
df_indiv_clusters_hm <- data.frame(
  Cluster = c(rep(1, length(indiv_clust[1:14])), 
              rep(2, length(indiv_clust[15:28])),
              rep(3, length(indiv_clust[29:43])),
              rep(4, length(indiv_clust[44:53]))),
  Patient = c(colnames(matrix_expr_cta)[indiv_clust[1:14]],
            colnames(matrix_expr_cta)[indiv_clust[15:28]],
            colnames(matrix_expr_cta)[indiv_clust[29:43]],
            colnames(matrix_expr_cta)[indiv_clust[44:53]]))

# Save
#write.table(df_indiv_clusters_hm, file = "../results/clusters_cta_signif_coxph_osteo_53.tsv", sep = "\t", quote = FALSE, row.names = FALSE)
```

```{r, fig.cap="Heatmap of CTAs that impact survival with k = 2 (n = 53)"}
row_dend <- row_dend(heatmap_cta_signif)

# Cut in 2 clusters (HOT/COLD)
row_clusters <- cutree(as.hclust(row_dend), k = 2)

# Cluster annotation
df_cta_clusters_hm <- as.data.frame(row_clusters)
...
#dev.off()
#write.table(df_cta_clusters_hm, file = "../results/clusters_cta_heatmap_signif_coxph_osteo_53.tsv", sep = "\t", quote = FALSE)
```
...
```{r, fig.cap="Heatmap with k-means clustering (k = 3) of immune cells expression (n = 53)"}
...
Given the presence of moderately infiltrated tumors, we perform k-means clustering with k = 4.
```{r, fig.cap="Heatmap with k-means clustering (k = 4) of immune cells expression (n = 53)"}
#pdf("../results/figures/heatmaps/heatmap_kmeans_4.pdf", width = 8, height = 6)
set.seed(1)
...
## 5) Adding metadata
```{r, fig.height=8, fig.width=8, fig.cap="Heatmap with metadata (n = 53)"}
# Select colors
...
# III. Exploring the relationship between the expression of CTAs and immune cell infiltration
...
```{r, fig.height=6, fig.width=8, fig.cap="Heatmap of CTA that impact survival analysis and immune cells expression (n = 53)"}
# CTA heatmap
...
## 2) Correlation
```{r, fig.width=8, fig.cap="Correlation heatmap (n = 53)"}
rownames(df_imm_z_scores) <- df_imm_z_scores$Signature
df_imm <- df_imm_z_scores[, -1]
cor_matrix <- cor(t(df_imm), t(matrix_expr_cta), method = "pearson")
...
# Heatmap
#pdf("../results/figures/correlation_matrix_all_CTA_osteo.pdf", height = 6, width = 20)
colors <- colorRampPalette(c("#75E05A", "white", "#EA4343"))(100)
heatmap_all_cta_corr_osteo <- Heatmap(cor_matrix, 
        cluster_rows = TRUE,    
        cluster_columns = TRUE,
        cluster_column_slices = TRUE,
        clustering_distance_columns = "euclidean",
...
heatmap_all_cta_corr_osteo <- draw(heatmap_all_cta_corr_osteo)
#dev.off()
```

```{r, fig.width=8, fig.cap="Correlation heatmap k = 2 (n = 53)"}
# Store clusters HOT and COLD
col_dend <- column_dend(heatmap_all_cta_corr_osteo)

# Cut in 2 clusters (HOT/COLD)
col_clusters <- cutree(as.hclust(col_dend), k = 2)

# Cluster annotation
df_cta_clusters_hm <- as.data.frame(col_clusters)
df_cta_clusters_hm$Cluster <- ifelse(col_clusters == 1, "HOT", "COLD")


#pdf("../results/figures/correlation_matrix_all_CTA_osteo_2_clust.pdf", height = 6, width = 20)
heatmap_corr_cta_k2 <- Heatmap(cor_matrix, 
        cluster_rows = TRUE,    
        cluster_columns = TRUE,
...
heatmap_corr_cta_k2 <- draw(heatmap_corr_cta_k2)
#dev.off()
# Save
#write.table(df_cta_clusters_hm, file = "../results/clusters_cta_pearson_53_k2.tsv", sep = "\t", quote = FALSE)
```

```{r, fig.width=8, fig.cap="Correlation heatmap k = 3 (n = 53)"}
# Store clusters HOT and COLD
col_dend <- column_dend(heatmap_all_cta_corr_osteo)
...
#pdf("../results/figures/correlation_matrix_all_CTA_osteo_3_clust.pdf", height = 6, width = 20)
Heatmap(cor_matrix, 
        cluster_rows = TRUE,    
        cluster_columns = TRUE,
        column_split = col_clusters,
